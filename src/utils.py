...
def make_dataset(path_or_data, w2id=None, class2id=None, vocab_size=40000, min_freq=1, n_encoder=2, to_kana=False, emb=None, is_train=True):
    """
    example return:
        dataset = [
    ...
    if is_train:
        # 並列処理で高速化
        splited_lines = Parallel(n_jobs=-1, verbose=3)([delayed(split_text)(line, to_kana) for line in tqdm(lines)])
        left_words = Parallel(n_jobs=-1, verbose=3)([delayed(preprocess_text)(line[0], to_kana, do_split=True) for line in tqdm(splited_lines) if line[0] is not None])
        targets = [line[1] for line in tqdm(splited_lines) if line[1] is not None]
        right_words = Parallel(n_jobs=-1, verbose=3)([delayed(preprocess_text)(line[2], to_kana, do_split=True) for line in tqdm(splited_lines) if line[2] is not None])
    else:
